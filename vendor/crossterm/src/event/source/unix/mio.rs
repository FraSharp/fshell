                            ...
                            match self.tty_fd.read(&mut self.tty_buffer) {
                                Ok(read_count) => {
                                    if read_count > 0 {
                                        ...
                                        // A zero-byte read on the terminal is EOF. In
                                        // particular, a PTY master closing leaves this
                                        // descriptor readable forever; treating EOF as
                                        // another read attempt spins this thread at 100% CPU.
