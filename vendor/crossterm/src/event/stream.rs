    ...
    fn default() -> Self {
        ...
        thread::spawn(move || {
            while let Ok(task) = receiver.recv() {
                loop {
                    // Wake the stream consumer for both events and input-source
                    // errors. In particular, EOF must escape this worker instead
                    // of being retried forever after a PTY disconnect.
