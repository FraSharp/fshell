    ...
    match subcmd.as_str() {
        ...
        "ls" | "list" => {
            ...
            rt.block_on(async {
                let stream = UnixStream::connect(&socket_path)
                    .await
                    .map_err(|e| ShellError::from(format!("Daemon not running: {e}")))?;
                ...
                sink.send(Frame::from_client(&ClientMessage::ListSessions))
                    .await
                    .map_err(|e| ShellError::from(format!("IPC send error: {e}")))?;
                ...
                while let Some(res) = stream_rx.next().await {
                    match res {
                        ...
                        Err(e) => return Err(ShellError::from(format!("IPC recv error: {e}"))),
                    }
                }
                Ok(())
            })?;
        ...
        "kill-session" | "kill" => {
            let target = args_iter
                .next()
                .ok_or_else(|| ShellError::from("Usage: mux kill-session <name>".to_string()))?
                .as_str()
                .unwrap_or("")
                .to_string();
        ...
        "select-window" | "select" => {
            let index: u32 = args_iter
                ...
                .ok_or_else(|| ShellError::from("Usage: mux select-window <index>".to_string()))?;
...
fn send_ipc_cmd(msg: ClientMessage) -> Result<(), ShellError> {
    let socket_path = get_socket_path();
    let rt = tokio::runtime::Builder::new_current_thread()
        .enable_all()
        .build()
